...
class Baseline(nn.Module):
            ...
            'loss_kwargs':
                {
                    ...
                    'arcface': {
                        'margin': cfg.MODEL.LOSSES.ARCFACE.MARGIN,
                        'gamma': cfg.MODEL.LOSSES.ARCFACE.GAMMA,
                        'scale': cfg.MODEL.LOSSES.ARCFACE.SCALE
                    }
                }
        }
    ...
    def losses(self, outputs, gt_labels):
        ...
        if 'Arcface' in loss_names:
            arcface_kwargs = self.loss_kwargs.get('arcface')
            loss_dict['loss_arcface'] = arcface_loss(
                pred_features,
                gt_labels,
                arcface_kwargs.get('margin'),
                arcface_kwargs.get('gamma'),
            ) * arcface_kwargs.get('scale')
