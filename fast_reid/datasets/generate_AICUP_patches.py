...
def make_parser():
    ...
    parser.add_argument("--data_path", default=r"train", help="path to AICUP train data")
    parser.add_argument("--save_path", default=r"fast_reid/datasets", help="Path to save the MOT-ReID dataset")
    parser.add_argument('--imgsz', type=tuple, default=(720, 1280), help='img size, (height, width)')
    parser.add_argument('--train_ratio', type=float, default=0.84, help='The ratio of the train set when splitting the train set and the test set')
...
def main(args):
    ...
    save_path = os.path.join(args.save_path, 'AICUP-ReID_example')
    ...
    total_files = sorted(glob.glob(os.path.join(args.data_path, 'images-example', '*')))
    ...
    for time_path in total_files:
        for img_path in tqdm(glob.glob(os.path.join(time_path, '*')), desc=f'convert {time_path}'):
            ...
            with open(os.path.join(args.data_path, 'labels-example', timestemp, f'{img_name}.txt'), 'r') as gt_file:
