...
class BoTSORT(object):
    def __init__(self, args, frame_rate=30, night_t=False):
        ...
        # ReID module
        if night_t:
            self.proximity_thresh = args.proximity_night_thresh
            self.appearance_thresh = args.appearance_night_thresh
            print("nightMODEEEEEE ONNNNNN")
        else:
        ...
        if args.with_reid:
            if night_t:
                self.encoder = FastReIDInterface(args.fast_reid_config, args.fast_reid_nightweights, args.device)
                print("NNNNIIIIIGGGGHHHTTTTT")
            else:
                self.encoder = FastReIDInterface(args.fast_reid_config, args.fast_reid_weights, args.device)

        self.gmc = GMC(method=args.cmc_method, verbose=[args.name, args.ablation])
        self.night_t = night_t
